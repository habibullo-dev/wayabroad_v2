...
| **M3 — Probability Engine v1** | ✅ Done | Engine (ported + tested) + no-login free check + shortlist odds + PostHog events; **verified-DB overlay** live — `universities.verified` JSONB (50 enriched from `files/`), provenance-gated cost panel (verified✓ / estimate / waiver→Free) + verified scholarships |
...
- **2026-05-30 (M3)** — **Verified-DB overlay shipped.** Schema: one additive nullable `universities.verified` JSONB column (migration `20260530150000`), absorbing the heterogeneous provenance shapes without touching the existing estimate columns (fully reversible). Loader `pnpm load:verified` (`scripts/load-verified.ts`) normalizes `files/universities_verified_v2.json` — a recursive walker pulls per-semester KRW tuition from `number`/`[min,max]`/`by_field`/`by_college` shapes (band-gated 1M–12M, skip-subtrees for per-year/USD/fee keys), detects full intl waivers (`effective_for_admitted_intl: 0` → Free), and matches to live rows by gentle name + POSTECH/UNIST aliases. **All 50 live universities enriched** (the other 50 verified records have no live row yet). Frontend: `lib/data/verified.ts` (`getVerified` sanitizes every leaf to a primitive; `fieldTrust` gating) + a provenance-aware cost panel and scholarships card. **Gating is the methodology rule in code:** only `verified_official` renders as confirmed fact (green ✓ + source + date); `verified_thirdparty`/`estimated_*` render as a labeled "Estimate — confirm" (amber + source); `pending`/absent fall back to the existing estimate columns. 9/50 tuitions are official; KAIST/POSTECH read as **Free** (waiver).
...
- **M3 (verified-DB overlay) — Codex review, all 5 findings accepted/fixed.** Confirmed external links use
  `rel="noopener noreferrer"` and the regenerated types carry `verified: Json | null`. Fixed: (High) a
  `pending` tuition figure could render as fact and suppress the estimate fallback — overlay use is now
  gated on `fieldTrust !== "none"` (chip, "Free", source, and footer all key off the *shown* trust); (High)
  the scholarships overlay ignored status — now status-gated (pending → seed fallback, non-official →
  "Estimate" + neutral "Source" link); (Medium) the tuition walker could let a per-year value nested under a
  skipped parent slip through — it now blocks the whole subtree under a skip-key; (Medium) waiver "Free" is
  now status-gated; (Medium) `getVerified` deeply sanitizes every leaf to a primitive so a malformed blob
  can't reach React. Verify green: typecheck, lint, 21 tests, build; re-loaded data spot-checked (9 official,
  0 unparsed, KAIST/POSTECH Free).
